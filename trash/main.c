                          ...
                          uint16_t addr, uint16_t *value)
{
    ...
    if (got != want)
        return 0; /* timeout or short reply                 */
    ...
    if (rsp[0] != slave)
        return 0;
    if (rsp[1] != fc)
        return 0; /* could be exception (fc | 0x80)    */
...
static int drm_init(void)
{
    ...
    if (drm_fd < 0)
    {
        LOG_ERROR("DRM open: %s", strerror(errno));
        return -1;
    }
    ...
    for (uint32_t i = 0; i < res.count_connectors && !found; i++)
    {
        ...
        if (!drm_crtc_id)
            drm_crtc_id = crtc_ids[0];
        found = 1;
    }
    if (!found)
    {
        LOG_ERROR("No connected DRM display");
        return -1;
    }
    ...
    {
        LOG_ERROR("DRM create dumb: %s", strerror(errno));
        return -1;
    }
    drm_size = cr.size;
    ...
    {
        LOG_ERROR("DRM setcrtc: %s", strerror(errno));
        return -1;
    }
...
static int touch_init(void)
{
    ...
    for (int i = 0; devs[i]; i++)
    {
        ...
        {
            touch_fd = fd;
            LOG_INFO("Touch: %s", devs[i]);
            return 0;
        }
        close(fd);
    }
    LOG_WARN("No touch device");
    return -1;
}
...
static void disp_status(int cycle, int mb_ok, int mq_ok, int success, int total)
{
    ...
    char bshort[32];
    strncpy(bshort, cfg.mqtt_broker, 19);
    bshort[20] = 0;
    strcat(bshort, "...");
    ...
    if (filled > 0)
        fb_rect(8, 148, filled, 14, COL_GREEN);
    ...
    for (int i = 0; i < point_count && shown < 40 && y < DISP_H - 36; i++)
    {
        ...
        if (col == 0)
        {
            fb_hline(4, y + 11, DISP_W - 8, COL_DKGRAY);
            y += 14;
        }
        shown++;
    }
    ...
    for (int i = 0; i < NNUM; i++)
    {
        ...
        if (ui_button(60, ry + 14, 60, 32, "  -  ", COL_DKBLUE, COL_WHITE))
        {
            *nums[i].val -= nums[i].step;
            if (*nums[i].val < nums[i].lo)
                *nums[i].val = nums[i].lo;
        }
        ...
        {
            *nums[i].val += nums[i].step;
            if (*nums[i].val > nums[i].hi)
                *nums[i].val = nums[i].hi;
        }
    }
    int sy = 50 + NNUM * 60 + 20;
    fb_hline(0, sy, DISP_W, COL_DKGRAY);
    sy += 8;
    fb_str_c(sy, "Current Configuration", COL_PURPLE, COL_BLUE, 1);
    sy += 16;
    fb_hline(0, sy, DISP_W, COL_DKGRAY);
    sy += 8;
    fb_str(8, sy, "MQTT Broker:", COL_GRAY, COL_BLUE, 1);
    sy += 14;
    char bshort[48];
    strncpy(bshort, cfg.mqtt_broker, 47);
    bshort[47] = 0;
    fb_str(16, sy, bshort, COL_CYAN, COL_BLUE, 1);
    sy += 16;
    fb_str(8, sy, "MQTT User:", COL_GRAY, COL_BLUE, 1);
    fb_str(80, sy, cfg.mqtt_user, COL_CYAN, COL_BLUE, 1);
    sy += 14;
    fb_str(8, sy, "Modbus Port:", COL_GRAY, COL_BLUE, 1);
    fb_str(84, sy, cfg.modbus_port, COL_CYAN, COL_BLUE, 1);
    sy += 16;
    fb_hline(0, sy, DISP_W, COL_DKGRAY);
    sy += 8;
    fb_str_c(sy, "Edit broker/user/pass via SSH:", COL_DKGRAY, COL_BLUE, 1);
    sy += 12;
...
static int parse_registers(void)
{
    ...
    while (fgets(line, sizeof(line), f) && idx < MAX_POINTS)
    {
        ...
        if (cr >= 0 && cr < nc)
        {
            ...
            for (int i = 0; rt[i]; i++)
                rt[i] = tolower((unsigned char)rt[i]);
            if (strstr(rt, "coil"))
                points[idx].reg_type = REG_COIL;
            else if (strstr(rt, "discrete"))
                points[idx].reg_type = REG_DISCRETE;
            else if (strstr(rt, "input"))
                points[idx].reg_type = REG_INPUT;
        }
...
static void on_connect(struct mosquitto *m, void *ud, int rc)
{
    ...
    if (rc == 0)
        ...
        LOG_ERROR("MQTT failed (code %d)", rc);
}
