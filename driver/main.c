/* ============================================================
 * All original code unchanged EXCEPT:
 *  - Removed: #include <modbus/modbus.h>
 *  - Removed: modbus_t *mb_ctx
 *  - Added:   #include "modbus_raw.h"  (raw read/write on fd)
 *  - Changed: mb_connect() / read_point() to use raw fd
 *  - DRM: drop SET_MASTER (causes permission denied on A7)
 * ============================================================ */
...
/* ============================================================================
 * COLOUR + FONT  (unchanged from original)
 * ========================================================================== */
...
/* ── KEY CHANGE: raw fd instead of modbus_t* ─────────────────────────────── */
static int               mb_fd          = -1;   /* /dev/modbus_uart fd      */
...
/* ============================================================================
 * DRM — removed DRM_IOCTL_SET_MASTER (causes Permission denied on A7)
 * ========================================================================== */
...
static int drm_init(void)
{
    ...
    /* NOTE: Removed DRM_IOCTL_SET_MASTER — not needed when running as root
       and causes "Permission denied" when another process holds master    */
...
static int touch_init(void)
{
    ...
    for (int i=0;devs[i];i++){
        ...
        if(bits[ABS_X/8]&(1<<(ABS_X%8))){touch_fd=fd;
            LOG_INFO("Touch: %s",devs[i]);return 0;}
        close(fd);
    }
...
{ if(c<0x20||c>0x7E)c='?';
  ...
  for(int col=0;col<5;col++){uint8_t line=g[col];
    for(int row=0;row<7;row++) fb_rect(x+col*s,y+row*s,s,s,(line>>row)&1?fg:bg);}
  return x+(5+1)*s; }
...
{ int x=(DISP_W-fb_strw(s,sc))/2; fb_str(x,y,s,fg,bg,sc); }
static int ui_button(int x,int y,int w,int h,const char *label,uint16_t bg,uint16_t fg)
{ fb_rect(x,y,w,h,bg); fb_border(x,y,w,h,fg);
  fb_str(x+(w-fb_strw(label,1))/2,y+(h-7)/2,label,fg,bg,1);
  return touch_in_rect(x,y,w,h); }

/* ============================================================================
 * STATUS + SETTINGS SCREENS — unchanged from original
 * ========================================================================== */
...
static void disp_settings(void)
{
    ...
    for(int i=0;i<NNUM;i++){
        ...
        if(ui_button(60,ry+14,60,32,"  -  ",COL_DKBLUE,COL_WHITE)){
            *nums[i].val-=nums[i].step; if(*nums[i].val<nums[i].lo)*nums[i].val=nums[i].lo;}
        ...
        if(ui_button(360,ry+14,60,32,"  +  ",COL_DKBLUE,COL_WHITE)){
            *nums[i].val+=nums[i].step; if(*nums[i].val>nums[i].hi)*nums[i].val=nums[i].hi;}
    }
...
/* ============================================================================
 * MODBUS — raw fd, no libmodbus context
 * ========================================================================== */

static int mb_connect(void)
{
    LOG_INFO("Modbus: opening %s (raw fd, no termios)", cfg.modbus_port);
    mb_fd = open(cfg.modbus_port, O_RDWR);
    if (mb_fd < 0) {
        LOG_ERROR("open %s: %s", cfg.modbus_port, strerror(errno));
        return 0;
    }
    LOG_INFO("Modbus fd=%d ready", mb_fd);
    return 1;
}
...
static int read_point(ModbusPoint *pt)
{
    ...
    for (int retry = 0; retry < MAX_RETRIES; retry++) {
        ...
        switch (pt->reg_type) {
            case REG_COIL:
                rc = mb_read_bits(mb_fd, cfg.modbus_slave,
                                  pt->address, 1, &bit);
                if (rc == 1) { pt->value = bit; pt->valid = 1; return 1; }
                break;
            case REG_DISCRETE:
                rc = mb_read_input_bits(mb_fd, cfg.modbus_slave,
                                        pt->address, 1, &bit);
                if (rc == 1) { pt->value = bit; pt->valid = 1; return 1; }
                break;
            case REG_INPUT:
                rc = mb_read_input_registers(mb_fd, cfg.modbus_slave,
                                             pt->address, 1, &reg);
                if (rc == 1) { pt->value = reg; pt->valid = 1; return 1; }
                break;
            default:
                rc = mb_read_registers(mb_fd, cfg.modbus_slave,
                                       pt->address, 1, &reg);
                if (rc == 1) { pt->value = reg; pt->valid = 1; return 1; }
                break;
        }
...
/* ============================================================================
 * SIGNAL + BACKGROUND THREAD — unchanged
 * ========================================================================== */
...
int main(void)
{
    ...
    /* ── KEY CHANGE: open raw fd, no termios/libmodbus ─────────────────── */
