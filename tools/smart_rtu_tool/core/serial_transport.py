...
class SerialTransport(Transport):
    ...
    def test_connection(self) -> TransportResult:
        try:
            ...
            ser.close()
            return TransportResult(True, f"Port {self.port} opened OK "
                                 f"({len(data)} bytes seen on wake)")
        except Exception as e:
            return TransportResult(False, f"Could not open {self.port}: {e}")
    ...
    def push_text(self, content: str, remote_path: str,
        ...
        try:
            ...
            if marker.encode() in buf:
                return TransportResult(True, f"{remote_path.rsplit('/', 1)[-1]} written via serial console to {remote_path}")
            ...
            return TransportResult(False, f"Serial push failed: {e}")

    def push_file(self, local_path: str, remote_path: str,
                  username: str = "root", password=None,
                  **kwargs) -> TransportResult:
        """Reads a local file as text and uploads via console heredoc."""
        try:
            with open(local_path, "r") as f:
                content = f.read()
        except Exception as e:
            return TransportResult(False, f"Cannot read local file {local_path}: {e}")
        return self.push_text(content, remote_path, username, password)
    ...
    def push_combined_config(self, csv_content: str, json_content: str,
        ...
        if not r2.ok:
            return r2
        return TransportResult(True, "smart_rtu_config.csv and smart_rtu_config.json "
                                   "both written via serial console")

    def read_text(self, remote_path: str, username: str = "root",
                   password: Optional[str] = None, **kwargs) -> TransportResult:
        ...
        try:
            ...
            return TransportResult(False, f"Serial read failed: {e}")
    ...
    def delete_remote(self, remote_path: str, username: str = "root",
        ...
        try:
            ...
            return TransportResult(False, "No completion marker seen — check console connection.")
        except Exception as e:
            return TransportResult(False, f"Serial delete failed: {e}")
